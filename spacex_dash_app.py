...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.pie(filtered_df, values='class', names='Launch Site', title='Success Rate for All Launch Sites')
    else:
        # Pie chart for a specific launch site
        filtered_df = filtered_df[filtered_df['Launch Site'] == entered_site]
        counts = filtered_df['class'].value_counts()
        fig = px.pie(values=counts, names=['Failure', 'Success'], title=f'Success Rate for {entered_site}')
    return fig
